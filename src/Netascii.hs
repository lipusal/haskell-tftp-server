...
encodeLineEndings line = if BS.null line then line else BS.concat [processedPrefix, processedSuffix] where
    (prefix, suffix) = BS.break isLineBreak line
    (processedPrefix, charsConsumed) = encodeChunk prefix suffix
    processedSuffix = if BS.null suffix then suffix else encodeLineEndings safeSuffix
...
encodeChunk prefix suffix = if BS.null suffix then (prefix, 0) else (BS.concat[prefix, processedSuffix], charsConsumed) where
